...
int ADFdbm = 2; // Variable to configure the output-power of the ADF4351 - Default: +5dBm
...
case STATE_NORM_OP:
	...
	ADFdbm = 2;              // 3bit  Output-Power set to +5dBm
...
default:
	...
	ADFdbm = 2;              // Output-Power set to -4dBm
	D_RF_ena = 1; // 1bit  OutPwr 1=on           0 = off  Outport Null freischalten
	D_auxOutEna = 1; // 1bit  aux OutEna 1=on       0 = off  Outport Aux freischalten
	D_auxOutSel = 1;         // 1bit  aux OutSel
	D_MTLD = 0; // 1bit | This controls the "Mute til lock detect"-Function of the chip. It disables the output if the internal PLL has not locked.
...
void WriteADF2(int idx) { // make 4 byte from integer for SPI-Transfer
	byte buf[4];
	for (int i = 0; i < 4; i++) {
...
void ConvertFreq(double freq, unsigned long R[]) {
	...
	R[4] = (unsigned long) (4 + D_out_PWR * pow(2, 3) + D_RF_ena * pow(2, 5)
			+ D_auxOutPwr * pow(2, 6) + D_auxOutEna * pow(2, 8)
			+ D_auxOutSel * pow(2, 9) + D_MTLD * pow(2, 10)
			+ D_VcoPwrDown * pow(2, 11) + B_BandSelClk * pow(2, 12)
			+ D_RfDivSel * pow(2, 20) + D_FeedBck * pow(2, 23));
...
unsigned long DIGRAMP_Limit; // Limits of the Digital Ramp - Byte 7 - 4 = Upper Ramp Limit / Byte 3 - 0 = Lower Ramp Limit
unsigned long DIGRAMP_Step; // Step of Digital Ramp - Byte 7 - 4 = Decrement Step Size / Byte 3 - 0 = Increment Step Size
unsigned int DIGRAMP_Rate; // Rate of Diigtal Ramp - Byte 3 - 2 = Negative slope rate / Byte 1 - 0 = Positive Slope Rate
...
void DAC_Setup() {
	...
	Serial.print(dac_data[0], HEX);
	Serial.print(F(" "));
	Serial.print(dac_data[1], HEX);
	Serial.print(F(" "));
	Serial.println(dac_data[2], HEX);
	Serial.print(F("Channel A EEPROM: "));
	Serial.print(dac_data[3], HEX);
	Serial.print(F(" "));
	Serial.print(dac_data[4], HEX);
	Serial.print(F(" "));
	Serial.println(dac_data[5], HEX);

	Serial.print(F("Channel B: "));
	Serial.print(dac_data[6], HEX);
	Serial.print(F(" "));
	Serial.print(dac_data[7], HEX);
	Serial.print(F(" "));
	Serial.println(dac_data[8], HEX);
	Serial.print(F("Channel B EEPROM: "));
	Serial.print(dac_data[9], HEX);
	Serial.print(F(" "));
	Serial.print(dac_data[10], HEX);
	Serial.print(F(" "));
	Serial.println(dac_data[11], HEX);

	Serial.print(F("Channel C: "));
	Serial.print(dac_data[12], HEX);
	Serial.print(F(" "));
	Serial.print(dac_data[13], HEX);
	Serial.print(F(" "));
	Serial.println(dac_data[14], HEX);
	Serial.print(F("Channel C EEPROM: "));
	Serial.print(dac_data[15], HEX);
	Serial.print(F(" "));
	Serial.print(dac_data[16], HEX);
	Serial.print(F(" "));
	Serial.println(dac_data[17], HEX);

	Serial.print(F("Channel D: "));
	Serial.print(dac_data[18], HEX);
	Serial.print(F(" "));
	Serial.print(dac_data[19], HEX);
	Serial.print(F(" "));
	Serial.println(dac_data[20], HEX);
	Serial.print(F("Channel D EEPROM: "));
	Serial.print(dac_data[21], HEX);
	Serial.print(F(" "));
	Serial.print(dac_data[22], HEX);
	Serial.print(F(" "));
	Serial.println(dac_data[23], HEX);

	Serial.println(
			F(
					"Setting up the DAC for normal Operation: VREF = INTERNAL, GAIN = 1, POWERDOWN-RESISTOR = 100kOhm, OUTPUT-VALUE = 0x888"));
#endif
	/*
...
#define DAC_DEBUG
// Sets the AGC-Level voltage in mV <- The AGC tries to reach this voltage by adjusting the PIN-Attenuator
...
// Sets the AGC-Bias voltage in mV
